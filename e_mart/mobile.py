	...
	try:
		...
		invoices = frappe.get_all(
			...
			fields=[
				...
				"grand_total", "outstanding_amount", "status", "emi_schedule"
			],
			limit=limit,
...
def get_mobile_commission_data(employee):
	...
	try:
		commission_logs = frappe.get_all(
			...
			fields=[
				"name", "log_month", "total_commission_amount", 
				"total_sales_amount", "commission_percentage"
			],
	...
	try:
		...
		return {
			...
			"data": {
				...
				"remaining_amount": si.outstanding_amount
			}
		}
	except Exception as e:
		return {"status": "error", "message": str(e)}
...
def sync_mobile_data():
	...
	try:
		...
		updated_invoices = frappe.get_all(
			"Sales Invoice",
			filters={
				"modified": [">", last_sync] if last_sync else ["!=", ""]
			},
