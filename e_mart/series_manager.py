...
class SeriesManager:
	...
	def get_next_series(purchase_category, doctype="Purchase Invoice"):
		...
		try:
			...
			frappe.log_error(f"Series generation failed: {str(e)}", "Series Manager Error")
	...
	def _update_series_number(mapping_name, new_number):
		...
		try:
			...
			frappe.log_error(f"Failed to update series number: {str(e)}", "Series Manager Error")
	...
	def reset_series(purchase_category, new_start_number=1):
		...
		try:
			...
			frappe.log_error(f"Series reset failed: {str(e)}", "Series Manager Error")
	...
	def get_series_info(purchase_category):
		...
		try:
			...
			frappe.log_error(f"Failed to get series info: {str(e)}", "Series Manager Error")
			return None
...
class PurchaseSeriesHandler:
	...
	def on_submit(doc, method):
		...
		try:
			...
			frappe.log_error(f"Series generation on submit failed: {str(e)}", "Purchase Series Handler Error")
...
def reset_series(purchase_category, new_start_number=1):
	...
	try:
		...
		return {
			"status": "success",
			"message": f"Series reset for {purchase_category}"
		}
	except Exception as e:
		return {
			"status": "error",
...
def get_series_info(purchase_category):
	...
	try:
		...
		if info:
			...
			return {
				"status": "error",
				"message": f"No series mapping found for {purchase_category}"
			}
	except Exception as e:
		return {
			"status": "error",
